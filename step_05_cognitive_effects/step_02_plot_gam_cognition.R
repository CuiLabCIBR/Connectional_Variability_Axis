...
file_list <- c('gam_cog_hcpd_axis_slope', 'gam_cog_hcp_axis_slope')
i = 1
...
for (file_name in file_list)
{
  ...
  Fig <- switch(
   i,
   "1" = Fig + scale_y_continuous(expand = c(0, 0), limits = c(0.17, 0.28), breaks = seq(0.18, 0.27, by = 0.03))+
   ...
   "2" = Fig + scale_x_continuous(expand = c(0.01, 0), limits = c(100, 135), breaks = seq(100, 130, by = 10)) +
    scale_y_continuous(expand = c(0, 0), limits = c(0.224, 0.266), breaks = seq(0.23, 0.26, by = 0.01))
  )
...
write.table(results_table, paste0(working_dir, 'gam_cognition_stats.csv'), sep = ",", col.names = FALSE)
