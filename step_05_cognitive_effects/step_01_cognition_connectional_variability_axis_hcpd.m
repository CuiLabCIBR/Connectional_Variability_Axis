% ---------------------------------------------------------------------------------------------------------------
% This script was used to prepare HCP-D data of Figure 4
...
hcpd_cog = readtable([subinfo_dir 'hcpd_coginfo.csv']);
hcpd_cog.sex = categorical(hcpd_cog.sex);
hcpd_cog.sex = double(hcpd_cog.sex == 'M');

cog = hcpd_cog.fluid_cogcomp;
[cog_sort,cog_sort_idx] = sort(cog,'ascend');
hcpd_cog_sort = hcpd_cog(cog_sort_idx,:);
...
for i = 1:group_num
    idx = cog_group_idx{i};
    Age(i,1) = mean(hcpd_cog_sort.age(idx));
    Sex(i,1) = sum(hcpd_cog_sort.sex(idx))./length(idx);
    Cognition(i,1) = mean(hcpd_cog_sort.fluid_cogcomp(idx));
    HeadMotion(i,1) = mean(hcpd_cog_sort.fd_mean(idx));
end

save([working_dir 'hcpd_cognitive_info.mat'],'Age','Sex','HeadMotion','Cognition','cog_group_idx','cog_sort_idx')

%% calculate the fc variability and inter-edge Axis for each group
load([working_dir, 'hcpd_cognitive_info.mat'])
idx_hcpd_cog = hcpd_cog.idx_hcpd_cog;

fc_dir = [root_dir 'data/fc/schaefer400/'];
load([fc_dir '/FC_schaefer400_8run_hcpd.mat'],'all_session');

hcpd_fc = all_session;
hcpd_fc = hcpd_fc(idx_hcpd_cog,:,:); % get subjects with cog
...
writetable(tbl,[working_dir 'gam_cog_hcpd_axis_slope.csv'])
...
for i = 1:length(net_name_vec)
    ...
    writetable(tbl,[working_dir 'gam_cog_hcpd_edge'  num2str(i,'%.2d') '_'  net_name_vec{i} '.csv'])
end
