...
## hcp
NetInfo <- read.xlsx(paste0(working_dir, 'cog_effects_axis_hcp.xlsx'))
axis <- NetInfo$Value
...
p <- ggplot(data = df, aes(x = Net_All, y = axis, fill = Net_Single)) + 
  ...
  theme(axis.line = element_line(size = 0.3), aspect.ratio = 0.63) +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
  scale_y_continuous(expand = c(0, 0), limits = c(-7, 7), breaks = seq(-6, 6, by = 3)) +
  # theme(axis.text.y = element_blank(), axis.ticks.y = element_blank()) +
  theme(legend.position = "none")
p

ggsave(paste0(working_dir, 'cog_effects_axis_hcp.png'), plot = p, width = 13, height = 6, units = "cm", dpi = 1200)

## hcpd
NetInfo <- read.xlsx(paste0(working_dir, 'cog_effects_axis_hcpd.xlsx'))
axis <- NetInfo$Value
Net_All = NetInfo$Net_All
Net_Single = NetInfo$Net_Single
df <- data.frame(Net_All = Net_All, axis = axis, Net_Single = Net_Single)
...
ggsave(paste0(working_dir, 'cog_effects_axis_hcpd.png'), plot = p, width = 13, height = 6, units = "cm", dpi = 1200)