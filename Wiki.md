...
## Abstract
The human cerebral cortex exhibits intricate interareal functional synchronization at the macroscale, with substantial individual variability in these functional connections. However, the spatial organization of functional connectivity (FC) variability across the human connectome edges and its significance in cognitive development and psychopathology remain unclear. Here, we identified a connectional axis in the edge-level FC variability. The variability declined continuously along this axis from within-network to between-network connections, and from the edges linking association networks to those linking the sensorimotor and association networks. This connectional axis of functional variability is associated with spatial pattern of structural connectivity variability. Moreover, the connectional variability axis evolves in youth into an adult-like pattern with an increasing flatter axis slope. We also observed that the slope of connectional variability axis was positively related to the performance in the higher-order cognition, and was negatively related to both the baseline and longitudinal general psychopathology. Together, our results reveal a connectional axis in functional variability that is linked with structural connectome variability, refines during development, and holds relevance for cognition and psychopathology.
...
(1) step_01_calculate_fc_schaefer400.m
> This script is used to calculate the FC based on the `Schaefer-400` atlas. The HCP minimal preprocessed fMRI data were post-processed using **xcp-d** (https://xcp-d.readthedocs.io/). Post-processed steps include nuisance regression, filtering and smooth. Post-processed data `*space-fsLR_atlas-Schaefer417_den-91k_bold.ptseries.nii` for each participant was used, and then we sorted the 400 cortical regions based on Yeo_7Networks. 
>
> For HCP-D, **Four rest runs** and **five task runs** were concatenated and separated into **eight runs** to estimate `inter-individual variability` and `intra-individual variability`, respectively. For HCP, four rest runs were concatenated and separated into **twelve runs** to estimate `inter-individual variability` and `intra-individual variability`, respectively.
>
...
(2) step_02_load_fc_schaefer400.m
...
> We plotted the inter-individual FC variability matrix at the nodal-level and network-level for both the HCP-D and HCP datasets. The alignment between the two datasets was measured by Spearman's rank correlation. We grouped all edges into three types, including within-sensorimotor (S-S), within-association (A-A), and between association-sensorimotor (A-S), and compared their mean differences in FC variability using a permutation test (10,000 iterations).

(2) step_02_ci_bootstrap.R

> The confidence interval of Spearman's rank correlation was estimated using a bootstrap approach. 

(3) step_03_density_plot_nodal_level.py
> Density plot for the alignment between FC variability of HCP-D and HCP at the nodal level. 

(4) step_04_scatterplot_network_level.R
> Scatter plot for the alignment between FC variability of HCP-D and HCP at the network level. 

(5) step_05_barplot_connectional_variability_axis_single_net.R
...
(6) step_06_barplot_connectional_variability_axis_all_net.R
...
(1) step_01_prepare_data_development_hcpd.m
> To explore the maturation of connectional variability axis in youth, we divided the HCP-D participants (aged 8–21 years old) into multiple subgroups and calculated the inter-individual variability matrix for each group. We balanced the number of groups and group sizes using a **sliding window** approach to ensure sufficient statistical power and accurate estimation of individual variability. Specifically, 416 participants from the HCP-D dataset were sorted in ascending order by age and then divided into overlapping groups. We set the window length to 50 participants with a step size of five participants, and obtained 74 groups.

(2) step_02_lme_development_sliding_windows.R
> The LME model was used to estimate the FC variability for each age group. 

(3) step_03_development_connectional_variability_axis.m
> First, with the individual FC variability matrix in HCP-YA as a reference, we calculated the `variability axis alignment` as Spearman’s rank correlation between the variability matrix from each of the 74 HCP-D groups and the HCP-YA reference variability matrix across edges.
>
> Next, we evaluated how the connectional axis of individual FC variability developed in youth. The connectional variability axis represents the ranking of the 21 within-network and between-network average variabilities. We calculated the slope of the connectional variability axis using linear regression to quantify the degree of the axis, and estimated the `axis slope` for each of the 74 groups. 

(4) step_04_plot_gam_development.R
> To model both linear and nonlinear developmental effects of `variability axis alignment` and `axis slope`, we used a generalized additive model (GAM) with penalized splines, which estimates nonlinearities using restricted maximum likelihood (REML) and penalizes nonlinearity to avoid overfitting data. GAM was fit with the variability axis alignment was modeled as the dependent variable. The group age was modeled as a smooth term while group sex and group in-scanner head motion as model covariates as follows:
>
> To model both linear and nonlinear developmental effects, we used a generalized additive model (GAM) with penalized splines, which estimates nonlinearities using restricted maximum likelihood (REML) and penalizes nonlinearity in order to avoid over-fitting the data. The mean age across participants was calculated for each age group and used in the GAM. The mean age was modeled using a penalized spline, while including sex and in-scanner head motion as model covariates, as follows:
> $$\text {Variability axis alignment} \sim s(\text {Age, } k=3)+\text {Sex}+\text {Motion}$$
>
...
### step_06_psychopathology_effects

This section was based on the ABCC dataset. This folder contains codes to generate figures and results of `Figure 5. The connectional variability axis is linked to both baseline and longitudinal general psychopathology`. We used the `p-factor`  to quantify the participants’ general psychopathology. The general psychopathology (p-factor) score was derived by CBCL items. This analysis included 4496 participants from the baseline and 4009 from the 2-year follow-up.

(1) step_01_load_fc_p_factor.m

> Load FC from participants with p-factor scores.

(2) step_02_fc_var_p_factor.m

> Using sliding window approach (window length = 500, step size = 50) and calculate the FC variability and the axis slope within each group. 

(3) step_03_gam_axis_slope_p_factor_500_50.R

> A GAM analysis was performed to evaluate the relationship between variability axis slope and p-factor scores while controlling the linear and nonlinear effects of age, as well as the effects of sex and in-scanner motion. The equation for GAM is as follows:
>
> $$\text {Variability axis slope} \sim \text {p-factor}+s(\text {Age, } k=3)+\text {Sex}+\text {Motion}$$
>
> Similar to the developmental analyses, the significance of the association between the axis slope and psychopathology was assessed using an ANOVA that compared the full GAM model to a nested, reduced model with no cognition term. To evaluate the effect size, we calculated the partial *R*<sup>2</sup> between the full GAM model and the reduced model and signed the partial *R*<sup>2</sup> using the sign of the psychopathology coefficient (T-value) from the full GAM model. 

(4) step_04_gam_edge_var_p_factor_500_50.R

> A similar GAM analysis was performed to evaluate the relationship between each of the 21 network-level FC variability and p-factor scores. 

(5) step_05_get_edge_fc_var_axis_500_50.m

> Get the file to plot the association between each of the 21 network-level FC variability and p-factor scores.

(6) step_06_barplot_axis_all_net.R

> Plot the association between each of the 21 network-level FC variability and p-factor scores.
...
> The scripts for supplementary analyses will be uploaded in the future...
