...
dataset_list <- c('hcp', 'hcpd')
atlas_list <- c('schaefer400', 'glasser360')
...
for (dataset in dataset_list) {
  ...
  for (atlas in atlas_list) {
    ...
    if (atlas == 'schaefer400') {
      fc_path <- paste0(data_dir, 'fc_', dataset_atlas, '_noGSR.mat')
      h5ls(fc_path)
      data <- h5read(fc_path, '/fc_data')
      
      lme_results <- data.frame(lme_ICC_2wayM(data, subID, session))
      writeMat(paste0(out_dir, 'lme_', dataset_atlas, '_noGSR.mat'), lme_results = lme_results)
    }
