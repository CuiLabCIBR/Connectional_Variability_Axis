...
y_name <- c('Connectional axis alignment','Connectional axis slope')
i = 1

for (file_name in file_list)
{
  gam.data <- read.csv(paste0(working_dir,file_name,'.csv'));
  gam.model <- gam(Axis ~ s(Age, k=3) + Gender + HeadMotion, method = "REML", data = gam.data);
  gam.model.results <- summary(gam.model)
  
  gam.nullmodel <- gam(Axis ~ Gender + HeadMotion, method = "REML", data = gam.data);
  ...
  lm.model <- lm(Axis ~ Age + Gender + HeadMotion, data = gam.data);
  ...
  Fig <- plot(v, xlab = "Age (years)", ylab = y_name[i],
    ...
    theme(axis.text=element_text(size=16, color='black'), axis.title=element_text(size=16), aspect.ratio = 0.9) +
    scale_x_continuous(expand = c(0.01, 0),limits = c(9, 22), breaks = seq(9, 22, by = 2))
  
  Fig <- switch(
    i,
    "1" = Fig + scale_y_continuous(expand = c(0, 0),limits = c(0.773, 0.825), breaks = seq(0.78, 0.82, by = 0.01)),
    "2" = Fig + scale_y_continuous(expand = c(0, 0),limits = c(0.272, 0.345), breaks = seq(0.28, 0.34, by = 0.02))
  )
