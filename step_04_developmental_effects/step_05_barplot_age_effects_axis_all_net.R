...
NetInfo <- read.xlsx(paste0(working_dir, 'age_effects_axis.xlsx'))
axis <- NetInfo$Value
...
p <- ggplot(data = df, aes(x = Net_All, y = axis, fill = Net_Single)) + 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(-0.45, 0.25), breaks = seq(-0.4, 0.2, by = 0.2)) +
  # theme(axis.text.y = element_blank(), axis.ticks.y = element_blank()) +
  theme(legend.position = "none")
p

ggsave(paste0(working_dir, 'age_effects_axis_hcpd.png'), plot = p, width = 13, height = 6, units = "cm", dpi = 1200)
